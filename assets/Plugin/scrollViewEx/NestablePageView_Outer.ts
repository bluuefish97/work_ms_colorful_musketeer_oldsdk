import { _decorator, Node,  EventTouch, UITransform, Event, PageView, ScrollView, Vec2, Vec3 } from 'cc';
...
@ccclass('NestablePageView_Outer')
export class NestablePageView_Outer extends PageView {
    @property(ScrollView)
    ...
    onLoad() {
        ...
        this.m_InnerScrollViews = this.m_InnerScrollViews.filter(scrollView => {
            if (GlobalTool.instanceOfInterface<INestable_Inner>(scrollView, "setOuterPageView")) {
                scrollView.setOuterPageView(this);
                return true;
            } else {
                return false;
            }
    ...
    _onTouchMoved(event: EventTouch, captureListeners:any) {
        ...
        if (this.content) {
            if (!this.isDifferentBetweenSettingAndPlan(this)) {
               // this._handleMoveLogic(touch);     //禁止翻页
            }
        }
